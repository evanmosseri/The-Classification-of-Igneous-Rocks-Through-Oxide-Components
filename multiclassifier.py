...
class RockClassifier:
	...
	def feed(csv, **kwargs):
		...
		try:
			RockClassifier.classify.rock_names = np.unique(
				stam["Rock Type"].values)
		except:
			RockClassifier.classify.rock_names = np.unique(
				stam[stam.keys()[0]].values)
		RockClassifier.classify.rock_types = RockClassifier.labels_to_keys(
			RockClassifier.classify.rock_names)
	...
	def s_k_fold(self, feature_mat, ylabels, n_folds=4):
		...
		feature_train, feature_test = [feature_mat[x]
									   for x in train], [feature_mat[x] for x in test]
		ylabels_train, ylabels_test = [ylabels[x]
									   for x in train], [ylabels[x] for x in test]
	...
	def split_data(self, v_type="k_fold", n_folds=4):
		if v_type == "split":
			train_data, test_data, train_labels, test_labels = self.data[0:int(len(self.data) / 2)], self.data[int(
				len(self.data) / 2):], self.labels[0:int(len(self.labels) / 2)], self.labels[int(len(self.labels) / 2):]
		elif v_type == "c_validation":
			train_data, test_data, train_labels, test_labels = cross_validation.train_test_split(
				self.data, self.labels, test_size=0.3, random_state=0)
		elif v_type == "k_fold":
			train_data, train_labels, test_data, test_labels = self.s_k_fold(
				self.data, self.labels, n_folds=n_folds)
	...
	def __init__(self, data, labels, split_type="k_fold", n_folds=4):
		...
		self.train_data, self.train_labels, self.test_data, self.test_labels = self.split_data(
			split_type, n_folds=n_folds)
	...
	class classify:
		results = []
		default_classifiers = [GaussianNB(), MultinomialNB(), SVC(kernel="linear", C=0.025), SVC(gamma=2, C=1), KNeighborsClassifier(n_neighbors=4), DecisionTreeClassifier(), AdaBoostClassifier(
		), RandomForestClassifier(), QDA(), LDA(), LogisticRegression(), GradientBoostingClassifier(), ExtraTreesClassifier(), RidgeClassifier(), PassiveAggressiveClassifier(), SGDClassifier()]
		...
		def __init__(self, target_names=None, fun=default_classifiers):
			...
			for i in fun:
				...
				custom_report = self.precision_breakdown(
					self.test_labels, y_pred, target_names)
				custom_report["overall_accuracy"] = pure_accuracy_rate
				self.results.append({"accuracy": pure_accuracy_rate, "report": report, "custom_report": custom_report, "y_pred": y_pred, "test_labels": self.test_labels,
									 "test_data": self.test_data, "classifier": clf, "confusion_matrix": cm, "y_confidence": y_confidence, "classifier_name": i.__class__.__name__})

		def save_classifiers(self, dir="./"):
			for i in self.results:
				joblib.dump(
					i["classifier"], "{}/{}.pkl".format(dir, i["classifier_name"]), compress=9)

		@staticmethod
		def precision_breakdown(labels, y_pred, target_names):
			precisions = {}
			for i in range(len(target_names)):
				target_equalities = [[labels[x], y_pred[x]]
									 for x in range(len(labels)) if labels[x] == i]
				hits = [x for x in target_equalities if x[0] == x[1]]
				precisions[target_names[i]] = len(
					hits) / float(len(target_equalities))
			return precisions
		...
		def confusion_matricies(self, plot=False, save=True, accuracy=None, directory="./"):
			...
			for i in range(len(results)):
				...
				im = ax0.imshow(normalize_mat(
					results[i]["confusion_matrix"]), interpolation='nearest', cmap='Reds')
				plt.xticks(np.arange(0, 4), self.rock_names)
				plt.yticks(np.arange(0, 4), self.rock_names)
				cbar = plt.colorbar(im)
				ax0.set_title("{} Confusion Matrix\n {} accuracy".format(
					results[i]["classifier_name"], results[i]["accuracy"]))
				if save == True:
					plt.savefig(
						"{}/{}-{}_confusion_matrix.png".format(directory, i + 1, results[i]["classifier_name"]))
			if plot == True:
				plt.show()
			return self
		...
		def plotAnalysis(self, cmap={1: "red", 2: "green", 3: "blue", 4: "orange"}, plot_vars=["SIO2", "AL2O3", "MGO"], plot=False, save=True, directory="./", pca=False):
			...
			for i in self.results:
				...
				x1, y1, z1 = [
					test_data[:, self.chemicals.tolist().index(x)] for x in plot_vars]
				...
				for i in range(len(hit_shapes_mat)):
					ax.scatter(
						x1[i], y1[i], zs=z1[i], c=rock_colors[i], marker=hit_shapes_mat[i], s=50)
				proxy_labels = [matplotlib.lines.Line2D([0], [0], linestyle="none", c='black', marker='o'), matplotlib.lines.Line2D(
					[0], [0], linestyle="none", c='black', marker='*')]
				for i in range(1, 5):
					proxy_labels.append(
						matplotlib.lines.Line2D([0], [0], linestyle="none", c=cmap[i], marker='o'))
				ax.legend(proxy_labels, ['Incorrectly Classified', 'Correctly Classified'] +
						  self.rock_names.tolist(), numpoints=1, prop={'size': 6}, bbox_to_anchor=(1.1, .1))
				...
				if save == True:
					plt.savefig(
						"{}/{}_plot.png".format(directory, analysis_type))
			if plot == True:
				plt.show()
			return self
		...
		def plotAnalysisPCA(self, cmap={1: "red", 2: "green", 3: "blue", 4: "orange"}, plot=False, save=True, directory="./"):
			...
			for c in range(len(results)):
				...
				for x in range(len(r_samples)):
					samples = [[i for i in r_samples[x] if i[3] == i[4]], [
						i for i in r_samples[x] if i[3] != i[4]]]
					try:
						ax.scatter(np.array(samples[0])[:, 0], np.array(samples[0])[:, 1], zs=np.array(
							samples[0])[:, 2], c=cmap[x + 1], marker='*')
						ax.scatter(np.array(samples[1])[:, 0], np.array(samples[1])[:, 1], zs=np.array(
							samples[1])[:, 2], c=cmap[x + 1], marker='o')
				...
				if save == True:
					plt.savefig(
						"{}/{}-{}_plot.png".format(directory, c, analysis_type))
			if plot == True:
				plt.show()
			return self
		...
		def bar_chart(self, title="Classifier Comparison", horizontal=True, show=False, filename="barchart.png"):
			...
			fig = plt.figure(figsize=(24.0, 10.0)) if not(
				horizontal) else plt.figure()
			ax = fig.add_subplot(1, 1, 1)
			res = np.array(
				self.get_results("classifier_name", "custom_report", sort=True))
			vals = res[:, 1].tolist()
			keys = vals[0].keys()
			width = .15
			rects = []
			...
			if horizontal:
				...
				for i in range(len(breakdown)):
					rects.append(ax.barh(
						ind + width * (i), breakdown[i], width, color=cmap[i + 1], hatch=hatch[i + 1]))
			...
			ax.legend((i[0] for i in rects), map(
				lambda x: x.title().replace("_", " "), keys))
			ax.set_title(title)
			ax.tick_params(
				axis='both', which='major', labelsize=8, direction='out')
			if show:
				plt.show()
			else:

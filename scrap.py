...
perm = columns[1:]
# rock_types = np.unique(full_data["ROCK NAME"].values)
...
for i in comb:
	...
	label = temp_data[:,0]
	dat = temp_data[:,1:]
	avg.append([params]+[RockClassifier(np.array(dat,dtype=np.float64),RockClassifier.numbered_labels(label,rock_types)).classify(rock_names,fun=classifiers).get_avg_performance()[0]])
...
pd.DataFrame(avg).to_csv("./data/perm_new.csv")
